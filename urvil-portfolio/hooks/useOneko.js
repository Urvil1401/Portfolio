...
// spritesheet embedded inline. The cat chases the cursor; click it for a heart.
  ...
  useEffect(() => {
    ...
    function idle() {
      ...
      if (
        ...
        if (nekoX < 32) choices.push("scratchWallW");
        if (nekoY < 32) choices.push("scratchWallN");
        if (nekoX > window.innerWidth - 32) choices.push("scratchWallE");
        if (nekoY > window.innerHeight - 32) choices.push("scratchWallS");
